...
import type { NodeBindings } from "./registry.ts";
...
export function unconsumedOverridesForProps(
  raw: FigmaInstanceRaw,
  ...
  for (const [nodeId, binding] of Object.entries(bindings)) {
    ...
    if (node?.content && node.content in props) consume(nodeId, "characters");
    if (node?.visible && node.visible in props) consume(nodeId, "visible");
    if (node?.paint && node.paint in props) consume(nodeId, "fills");
    if (node?.textStyle && node.textStyle in props)
      consume(nodeId, "textStyleId");
    for (const propName of Object.values(binding.component ?? {})) {
      if (propName in props) consume(nodeId, "componentProperties");
    }
  }
