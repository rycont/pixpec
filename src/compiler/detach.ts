...
import type { RawNode, RawOverride, RawSolidPaint } from '../dumper/raw-node.ts'
...
export function shouldDetach(inst: RawNode, entry: RegistryEntry): boolean {
  ...
  const masterRoot = variantKey ? entry.masterSnapshot[variantKey] : undefined
  if (!masterRoot) {
    throw new Error(
      `pixpec compile: registered component "${entry.componentName}" has no master-snapshot.json ` +
      `entry for variant key ${variantKey ?? '<unknown>'} (instance ${inst.id}). ` +
...
function isFieldCoveredByBinding(field: string, binding?: NodeBindingValue): boolean {
  if (!binding) return false
  if (field === 'characters' && binding.attr?.text) return true
  if (field === 'visible' && binding.attr?.visible) return true
  if (field === 'componentProperties' && binding.instanceProps && Object.keys(binding.instanceProps).length > 0) return true
  return false
}
...
function fieldValueEquivalent(field: string, inst: RawNode, master: RawNode): boolean {
  switch (field) {
    case 'fills':
      ...
      if (inst.type === 'VECTOR' || inst.type === 'BOOLEAN_OPERATION') return true
      return paintListEquivalent(inst.fills, master.fills)
    case 'strokes': return paintListEquivalent(inst.strokes, master.strokes)
    case 'strokeWeight': return inst.strokeWeight === master.strokeWeight
    case 'strokeAlign': return inst.strokeAlign === master.strokeAlign
    case 'textStyleId': return textStyleEquivalent(inst, master)
    case 'fontName': return JSON.stringify(inst.fontName) === JSON.stringify(master.fontName)
    case 'fontSize': return inst.fontSize === master.fontSize
    case 'fontWeight': return inst.fontWeight === master.fontWeight
    case 'lineHeight': return JSON.stringify(inst.lineHeight) === JSON.stringify(master.lineHeight)
    case 'letterSpacing': return JSON.stringify(inst.letterSpacing) === JSON.stringify(master.letterSpacing)
    case 'textCase': return inst.textCase === master.textCase
    case 'textDecoration': return inst.textDecoration === master.textDecoration
    case 'cornerRadius': return inst.cornerRadius === master.cornerRadius
    case 'opacity': return (inst.opacity ?? 1) === (master.opacity ?? 1)
    case 'rotation': return (inst.rotation ?? 0) === (master.rotation ?? 0)
    case 'boundVariables': {
...
function paintListEquivalent(a?: unknown, b?: unknown): boolean {
  const aP = firstSolid(a), bP = firstSolid(b)
  if (!aP && !bP) return true
  if (!aP || !bP) return false
  const aOp = aP.opacity ?? 1, bOp = bP.opacity ?? 1
  if (Math.abs(aOp - bOp) > 1e-3) return false
  return Math.abs(aP.color.r - bP.color.r) < 1e-3
...
function firstSolid(p: unknown): RawSolidPaint | null {
  ...
  for (const x of p) {
    if (x && typeof x === 'object' && (x as { type?: string }).type === 'SOLID' && (x as { visible?: boolean }).visible !== false) {
